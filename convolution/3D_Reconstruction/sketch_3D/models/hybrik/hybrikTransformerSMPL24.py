...
class HybrikTransformerSMPL24(nn.Module):
    def __init__(self, norm_layer=nn.BatchNorm2d, **kwargs):
        ...
        self.preact = backbone(f"resnet{34}")
        ...
        h36m_jregressor = np.load('data/J_regressor_h36m.npy')
        ...
        # mean shape
        init_shape = np.load('data/h36m_mean_beta.npy')
...
#         self.decleaf = nn.Linear(1024, 5 * 4)  # rot_mat quat
        ...
        self.decjoint = nn.Linear(self.feature_channel, 6)
        self.decshape = nn.Linear(self.feature_channel, 10)
        self.decphi = nn.Linear(self.feature_channel, 2)  # [cos(phi), sin(phi)]
        self.decleaf = nn.Linear(self.feature_channel, 1)  # rot_mat quat
        
        transformer_class = Transformer
        self.transformer = transformer_class(num_blocks=6, 
                                       num_layers=3, 
                                       layer_width=512,
                                       d_model=512,
                                       dropout=0.1, 
                                       ...
                                       num_heads=8)
        self.retain_elements_low = 0.2
        self.retain_elements_high = 1.0
    ...
    def forward(self, x):
        ...
        pred_uvd_jts_24 = self.decjoint(transformer_output[:, :24]).view(batch_size, -1)
        # pred_phi = self.decphi(transformer_output[:, 24:24+23])
        pred_leaf = self.decleaf(transformer_output[:, 24+23:24+23+3]).squeeze(2)
...
#             maxvals=maxvals
#         )
        pred_phi = rot6d_to_rotmat(pred_uvd_jts_24).view(batch_size, 24, 3, 3)
        return pred_phi, pred_shape, pred_leaf
