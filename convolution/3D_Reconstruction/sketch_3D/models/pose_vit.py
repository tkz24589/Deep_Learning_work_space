...
class PoseRegressor(nn.Module):
    def __init__(self, smpl_mean_params):
        ...
        self.pose_tranformer = Transformer(dim=1024 + 144 + 10 + 3, depth=4, heads=4,
                                           dim_head=16, mlp_dim=2048, out_dim=144 + 10 + 3)
        self.fc1 = nn.Linear(1024, 1024)
        self.drop1 = nn.Dropout()
        self.fc2 = nn.Linear(1024, 1024)
        self.drop2 = nn.Dropout()
        self.pose_docoder = nn.Linear(1024 + 144 + 10 + 3, 144)
        self.shape_docoder = nn.Linear(1024 + 144 + 10 + 3, 10)
        self.cam_docoder = nn.Linear(1024 + 144 + 10 + 3, 3)
    ...
    def forward(self, x):
        ...
        xc = torch.cat([x, init_pose, init_shape, init_cam], dim=1).unsqueeze(1)
        xc = self.pose_tranformer(xc).squeeze(1)

        x3 = torch.cat([x2, xc], dim=1)

        pred_pose = self.pose_docoder(x3)
        pred_shape = self.shape_docoder(x3)
        pred_cam = self.cam_docoder(x3)
...
class PoseVit(nn.Module):
    def __init__(self, smpl_mean_params):
        ...
        self.feature = NextViT(stem_chs=[64, 32, 64], depths=[3, 4, 10, 3], path_dropout=0.1)
        self.regressor = PoseRegressor(smpl_mean_params)
