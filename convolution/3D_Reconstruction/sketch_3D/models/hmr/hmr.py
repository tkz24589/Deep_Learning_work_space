...
class ThetaRegressor(LinearModel):
    def __init__(self, fc_layers, use_dropout, drop_prob, use_ac_func, iterations, batch_size):
...
class HMRNetBase(nn.Module):
    ...
    def _read_configs(self):
        # def _check_config():
            ...
            #     assert not enable_inter_supervions
            # else:
            #     msg = 'invalid encoder network, only {} is allowd, got {}'.format(args.allowed_encoder_net,
            #                                                                       encoder_name)
            #     sys.exit(msg)
        ...
        self.beta_count = 10
        self.smpl_model = 'data/smpl'
        self.smpl_mean_theta_path = ''
    ...
    def _create_sub_modules(self):
        ...
        '''
        # self.smpl = SMPL(self.smpl_model)
        ...
        iterations = 3
        self.regressor = ThetaRegressor(fc_layers, use_dropout, drop_prob, use_ac_func, iterations, self.batch_size)
        self.iterations = iterations
    ...
    def forward(self, inputs):
        if self.encoder_name == 'resnet50':
            ...
            return self._calc_detail_info(thetas)
            # feature = self.encoder(inputs)
            # thetas = self.regressor(feature)
            # detail_info = []
            # for theta in thetas:
    ...
    def _calc_detail_info(self, theta):
        ...
        # pose = rot6d_to_rotmat(pose).view(theta.shape[0], 24, 3, 3)
