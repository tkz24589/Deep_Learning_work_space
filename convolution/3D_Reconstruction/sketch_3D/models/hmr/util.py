...
def load_mean_theta():
    ...
    mean_values = np.load('data/smpl_mean_params.npz')
    mean_pose = mean_values['pose']
    pose = torch.from_numpy(mean_pose).unsqueeze(0)
    pose = rot6d_to_rotmat(pose).view(1, 24, 3, 3)
    mean_pose = gu.rotation_matrix_to_angle_axis(pose).reshape(72)
    mean_pose = mean_pose.detach().cpu().numpy()
    mean_shape = mean_values['shape']
    mean_cam = mean_values['cam']
