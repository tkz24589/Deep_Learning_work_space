...
from models.cmr import CMR
from models.cmr.mesh import Mesh
...
parser.add_argument('--checkpoint', default='logs/comparison/cmr/checkpoints/cmr-sketch-best.pt',
...
class Eval():
    ...
    def eval_look(self, checkpoint_filename):
        self.model = CMR(self.mesh, 5, 256,
                         pretrained_checkpoint=checkpoint_filename)
        self.model.eval()
        for i in range(2):
            if i == 0:
...
def run_evaluation(model, dataset_name, dataset, result_file,
    ...
    for step, batch in enumerate(tqdm(data_loader, desc='Eval', total=len(data_loader))):
        ...
        pred_vertices_smpl = torch.zeros_like(gt_vertices)
        images = batch['img'].to(device)
        curr_batch_size = images.shape[0]

        with torch.no_grad():
            pred_vertices, pred_vertices_smpl, pred_camera, pred_rotmat, pred_betas = model(images)
            pred_vertices_smpl = smpl_neutral(pred_rotmat, pred_betas).vertices
            mask_smpl, _ = renderer(pred_vertices_smpl, pred_camera, False)
            if show:
                cv2.imshow('w', images.cpu().numpy()[0][0])
                cv2.imshow('r', mask_smpl.cpu().numpy()[0]*255)
        ...
        if eval_pose:
            ...
            pred_keypoints_3d_smpl = pred_keypoints_3d_smpl[:, joint_mapper_h36m, :]
            ...
            for i in range(curr_batch_size):
                ...
                if eval_smpl:
                    ...
                    for c in range(2):
                        ...
                        fn[c] += (cgt & ~cpred).sum()
                    f1 = 2 * tp / (2 * tp + fp + fn)
                pred_mask_smpl = mask_smpl[i].cpu().numpy().astype(int)
                # box = get_box(pred_mask_smpl, False)
                # center, scale = location_from_box(box)
                # pred_mask_smpl = crop(pred_mask_smpl, center, scale, (224, 224))
                pred_mask_smpl = pred_mask_smpl > 0
                accuracy_smpl += (gt_mask == pred_mask_smpl).sum()
                pixel_count_smpl += np.prod(np.array(gt_mask.shape))
                for c in range(2):
                    cgt = gt_mask == c
                    cpred = pred_mask_smpl == c
                    tp[c] += (cgt & cpred).sum()
                    fp[c] += (~cgt & cpred).sum()
                    fn[c] += (cgt & ~cpred).sum()
                f1_smpl = 2 * tp / (2 * tp + fp + fn)

        # Shape evaluation (Mean per-vertex error)
        if eval_shape:
            se = torch.sqrt(((pred_vertices - gt_vertices) ** 2).sum(dim=-1)).mean(dim=-1).cpu().numpy()
            se_smpl = torch.sqrt(((pred_vertices_smpl - gt_vertices) ** 2).sum(dim=-1)).mean(
                dim=-1).cpu().numpy()
        ...
        if step % log_freq == log_freq - 1:
            if eval_pose:
                ...
                print('MPJPE SMPL: ' + str(1000 * mpjpe_smpl[:step * batch_size].mean()))
                print('Reconstruction Error SMPL: ' + str(1000 * recon_err_smpl[:step * batch_size].mean()))
                print()
            if eval_masks:
                if eval_smpl:
                    print('Accuracy: ', accuracy / pixel_count)
                    print('F1: ', f1.mean())
                print('Accuracy SMPL: ', accuracy_smpl / pixel_count_smpl)
                print('F1 SMPL: ', f1_smpl.mean())
                print()
            if eval_shape:
                if eval_smpl:
                    print('Shape Error: ' + str(1000 * shape_err[:step * batch_size].mean()))
                print('Shape Error (SMPL): ' + str(1000 * shape_err_smpl[:step * batch_size].mean()))
                print()
    ...
    if eval_pose:
        ...
        print('MPJPE SMPL: ' + str(1000 * mpjpe_smpl.mean()))
        print('Reconstruction Error SMPL: ' + str(1000 * recon_err_smpl.mean()))
        print()
    if eval_masks:
        if eval_smpl:
            print('Accuracy: ', accuracy / pixel_count)
    ...
    if eval_shape:
        ...
        print('Shape Error (SMPL): ' + str(1000 * shape_err_smpl.mean()))
        print()
