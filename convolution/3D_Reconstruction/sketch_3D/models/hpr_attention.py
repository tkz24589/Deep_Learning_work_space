...
class HPR(nn.Module):
    ...
    def forward(self, x, init_pose=None, init_shape=None, init_cam=None):
        ...
        # test attention
        # feature_copy_map = feature.cpu().numpy() * 255
        # x1_copy_map = x1.cpu().numpy() * 255
        # x2_copy_map = x2.cpu().numpy() * 255
        # x3_copy_map = x3.cpu().numpy() * 255
        # dconv = nn.ConvTranspose2d(2048, 1, kernel_size=64, stride=1, padding=0, bias=False).to('cuda')
        # feature_copy_map = dconv(feature.reshape(1, 2048, 1, 1))[0][0].cpu().numpy() * 255
        # x1_copy_map = dconv(x1.reshape(1, 2048, 1, 1))[0][0].cpu().numpy() * 255
        # x2_copy_map = dconv(x2.reshape(1, 2048, 1, 1))[0][0].cpu().numpy() * 255
        # x3_copy_map = dconv(x3.reshape(1, 2048, 1, 1))[0][0].cpu().numpy() * 255

        # feature_copy_map = cv2.resize(feature_copy_map, (224, 224))
        # x1_copy_map = cv2.resize(x1_copy_map, (224, 224))
        # x2_copy_map = cv2.resize(x2_copy_map, (224, 224))
        # x3_copy_map = cv2.resize(x3_copy_map, (224, 224))
        # cv2.imshow('logs/eval_example/feture_decov_map.png', feature_copy_map)
        # cv2.imshow('logs/eval_example/pose_decov_map.png', x1_copy_map)
        # cv2.imshow('logs/eval_example/shape_decov_map.png', x2_copy_map)
        # cv2.imshow('logs/eval_example/cam_decov_map.png', x3_copy_map)
        # cv2.waitKey(0)
        pred_pose = init_pose
        pred_shape = init_shape
        pred_cam = init_cam
        pred_pose = self.pose_regressor(torch.cat([x1, pred_pose], dim=1))
        if self.type != '3d':
...
if __name__== "__main__" :
    net = hpr_attention_net(smpl_mean_params="data/smpl_mean_params.npz")
