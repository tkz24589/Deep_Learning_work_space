...
class Visualizer(object):
    """
   Visualizer to visuzlie SMPL reconstruction output from HMR family (HMR, Sketch-SPIN, EFT)
    ...
    def visualize(self,
        ...
        if mesh_2d:
            obj_img = self.__render_pred_verts(input_img, pred_mesh_list)
            return obj_img
    ...
    def _visualize_screenless_naive(self, meshList, skelList=None, body_bbox_list=None, img_original=None, show_side = False, vis=False, maxHeight = 1080):
        ...
        for mesh in meshList:
            self.renderer.add_mesh(mesh['ver'], mesh['f'], color=(0.22, 0.25, 0.65))
