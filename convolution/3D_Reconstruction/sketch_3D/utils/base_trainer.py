...
class BaseTrainer(object):
    ...
    def __init__(self, options):
        ...
        if self.options.model_name == 'cmr':
            from models.cmr.eval_cmr import Eval
        if self.options.model_name == 'sketch_body_net' or self.options.model_name == 'hpr' or self.options.model_name == 'hpr_no_attention' or self.options.model_name == 'hpr_single' or self.options.model_name == 'hpr_single' or self.options.model_name == 'hpr_pointnet':
            from eval import Eval
        if self.options.model_name == 'hmr':
            from models.hmr.eval_hmr import Eval
        if self.options.model_name == 'spin':
            from models.spin.spin_eval import Eval
        if self.options.model_name == 'maed':
            from models.maed.maed_eval import Eval
        if self.options.model_name == 'hybrik':
            from models.hybrik.hybrik_eval import Eval
        self.eval = Eval(options)
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.epoch_count, self.options.num_epochs), total=self.options.num_epochs,
                ...
                if time.time() < self.endtime:
                    ...
                    if self.step_count % self.options.checkpoint_steps == 0:
                        self.saver.save_checkpoint(self.models_dict, self.optimizers_dict, epoch, step + 1,
                                                   ...
                                                   self.step_count,
                                                   loss / (len(self.train_ds) / self.options.batch_size))
                    ...
                    self.saver.save_checkpoint(self.models_dict, self.optimizers_dict, epoch, step,
                                               self.options.batch_size, train_data_loader.sampler.dataset_perm,
                                               self.step_count, loss / (len(self.train_ds) / self.options.batch_size))
                    tqdm.write('Checkpoint saved')
                    sys.exit(0)
            ...
            # if (epoch + 1) % 10 == 0:
            #     # if (epoch + 1) % 10 == 0 or loss < best_loss:
            #     best_loss = loss
            ...
            #                                self.options.batch_size, None, self.step_count, loss / ((self.train_ds) / self.batch_size))
            if (epoch+1) % self.options.sava_step == 0:
                checkpoint_filename = self.saver.save_checkpoint(self.models_dict, self.optimizers_dict, epoch + 1, 0,
                                                                 self.options.batch_size, None, self.step_count,
                                                                 loss / (len(self.train_ds) / self.options.batch_size))
                self.eval.eval_look(checkpoint_filename)
            loss = 0
        return
