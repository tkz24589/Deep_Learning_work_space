# Original code from Sketch-SPIN: https://github.com/nkolot/SPIN
...
def crop(img, center, scale, res, rot=0):
    ...
    if new_y[1] - new_y[0] != old_y[1] - old_y[0] or new_x[1] - new_x[0] != old_x[1] - old_x[0] or new_y[1] - new_y[
        0] < 0 or new_x[1] - new_x[0] < 0:
...
def process_image_bbox(img_original, bbox_XYWH, input_res=224):
    ...
    # viewer2D.ImShow(img, name='cropped', waitTime=0)        #224,224,3
...
def process_image(img_original, center, scale, input_res=224):
    """Read image, do preprocessing and possibly crop it according to the bounding box.
    If there are bounding box annotations, use them to crop the image.
    If no bounding box is specified but openpose detections are available, use them to get the bounding box.
    """
    ...
    bboxInfo = {"center": center, "scale": scale}
    return img, norm_img, boxScale_o2n, bboxTopLeft, bboxInfo


g_de_normalize_img = Normalize(
    mean=[-constants.IMG_NORM_MEAN[0] / constants.IMG_NORM_STD[0], -constants.IMG_NORM_MEAN[1] /
          constants.IMG_NORM_STD[1], -constants.IMG_NORM_MEAN[2] /
          constants.IMG_NORM_STD[2]], std=[1 / constants.IMG_NORM_STD[0], 1 / constants.IMG_NORM_STD[1], 1 /
                                           constants.IMG_NORM_STD[2]])
