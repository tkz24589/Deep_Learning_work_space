...
class general_pose_model(object):
    ...
    def save_keypoints(self, json_path, img_name, poses):
        # joints
        filename = json_path + '/' + img_name[0].split('/')[-1] + '_keypoints.json'
...
def body_pose(img_file):
    json_path = "datasets/openpose/mpi_inf_3dhp"
    modelpath = "/home/stu/PycharmProjects/opscoco/models"
    ...
    # file_name = pose_model.save_keypoints(json_path, str(img_file).split('.')[:-1], res_points)


if __name__ == '__main__':
    print("[INFO]Pose estimation.")
    img_path = "//datasets/data/h36m/S5/"
