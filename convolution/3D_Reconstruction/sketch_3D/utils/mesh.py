...
class Mesh(object):
    ...
    def __init__(self, filename='data/mesh_downsampling.npz',
        ...
        ref_vertices = self.smpl.v_template
        ...
        self.faces = torch.from_numpy(self.smpl.faces.astype(np.int64)).int().to(device)
    ...
    def reshape(self, pose, beta):
        ref_vertices = self.smpl(pose, beta).v_template
        center = 0.5 * (ref_vertices.max(dim=0)[0] + ref_vertices.min(dim=0)[0])[None]
        ref_vertices -= center
        ref_vertices /= ref_vertices.abs().max().item()

        self._ref_vertices = ref_vertices.to(pose.device)
        self.faces = self.smpl.faces.int().to(pose.device)
