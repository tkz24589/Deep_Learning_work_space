...
def draw_person_pose(orig_img, poses):
    ...
    for pose in poses.round().astype('i'):
        for i, ((x, y, v), color) in enumerate(zip(pose, joint_colors)):
            if v != 0:
                json_file['people'][0]['pose_keypoints_2d'].append(np.float(x))
                json_file['people'][0]['pose_keypoints_2d'].append(np.float(y))
                json_file['people'][0]['pose_keypoints_2d'].append(np.float(v))
                cv2.circle(canvas, (x, y), 3, color, -1)
    with open(filename, 'w') as file_obj:
        json.dump(json_file, file_obj)
    return canvas

def save_keypoints(json_path, img_name, poses):
    # joints
    ...
    for pose in poses.round().astype('i'):
        for i in pose:
            if i[2] != 0:
                ...
                json_file['people'][0]['pose_keypoints_2d'].append(np.float(i[2]))
    with open(filename, 'w') as file_obj:
        json.dump(json_file, file_obj)
        print(filename)
