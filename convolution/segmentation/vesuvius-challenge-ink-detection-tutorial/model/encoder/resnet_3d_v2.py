...
def resnet_3d(model_depth, inplanes, pretrained=True, **kwargs):
    assert model_depth in [10, 18, 34, 50, 101, 152, 200]

    if model_depth == 10:
        model = ResNet(BasicBlock, [1, 1, 1, 1], inplanes, **kwargs)
    elif model_depth == 18:
        model = ResNet(Bottleneck, [2, 2, 2, 2], inplanes, **kwargs)
    elif model_depth == 34:
    ...
    if pretrained:
        state_dict = torch.load("result/resnet3d-seg-2d/r3d18_K_200ep.pth")["state_dict"]
        conv1_weight = state_dict['conv1.weight']
        state_dict['conv1.weight'] = conv1_weight.sum(dim=1, keepdim=True)
        model.load_state_dict(state_dict, strict=False)
        print('load encoder weight')
    return model