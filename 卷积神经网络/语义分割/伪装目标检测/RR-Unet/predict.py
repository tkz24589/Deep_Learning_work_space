...
if __name__ == "__main__":
    ...
    file_names = os.listdir("data/JPEGImages")
    for img_name in file_names:
        img = Image.open("data/JPEGImages/" + img_name)
        W, H = img.size[0], img.size[1]
        model = 'result/logs/CASIA/Ringed_Res_Unet/PEE-CASIA-[val_dice]-0.9900-[train_loss]-0.0687.pkl'
        ...
        if not no_save:
            result = mask_to_image(mask).resize((W, H))

            if network == 'Unet':
                result.save('data/test_img/test/mask/' + img_name.split('.')[0] + '.png')
            elif network == 'Res_Unet':
                result.save('data/test_img/test/mask/' + img_name.split('.')[0] + '.png')
            else:
                result.save('data/test_img/test/newmask/' + img_name.split('.')[0] + '.png')